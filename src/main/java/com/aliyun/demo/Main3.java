...
public class Main3 {
    public static void main(String[] args) {
        ...
        try {
            ...
            factory.getConfiguration().addMapper(GradeMapper.class);
...
//插入
            ...
            GradeMapper.insert_grade(newGrade);
            session.commit();
            System.out.println("print after insert: " + newGrade.getId());
//输出插入的数据
            Grade Grade = GradeMapper.get_grade(newGrade.getId());
            System.out.println(Grade);

//更新 年纪加一
            System.out.println("grade + 1 : ");
            GradeMapper.update_grade();
            session.commit();
            System.out.println(Grade);

//更新 毕业            
            System.out.println("graduate : ");
            GradeMapper.update_graduated();
            session.commit();
            System.out.println(Grade);

//删除
            // GradeMapper.delete_grade(newGrade.getId());
            // session.commit();
    ...
    public static void printtop(GradeMapper GradeMapper) {
        List<Grade> Grades = GradeMapper.get_grade_paging(10, 0);
        for (Grade stu : Grades) {
            System.out.println(stu);
        }
    }

    public static void printById(GradeMapper GradeMapper, long id) {
        Grade Grade = GradeMapper.get_grade(id);
        if (Grade == null) {
